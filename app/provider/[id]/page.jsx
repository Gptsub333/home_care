...
// //   // Get user's address from localStorage or state
// //   const userAddress = localStorage.getItem('userAddress') || '';
  
// //   if (!userAddress) {
// //     alert("Please set your service address in your profile before booking.");
// //     return;
// //   }
...
// //         price: provider?.consultationFee || 0,
// //         patientNotes: notes || "",
// //         serviceAddress: userAddress, // Add this
// //         serviceLat: null, // Optional: add if you have coordinates
// //         serviceLng: null  // Optional: add if you have coordinates
// //       }),
// //     });
...
//     localStorage.setItem('userLastAddress', location.address);
//     localStorage.setItem('userLastLat', location.lat);
//     localStorage.setItem('userLastLng', location.lng);
//   };
...
//                         {/* Show compact address if selected and map hidden */}
...
//                       {/* Booking Summary */}
//                       {selectedService && selectedTime && locationData.address && (
...
//                               {locationData.address.substring(0, 50)}
...
export default function ProviderProfilePage() {
  ...
  useEffect(() => {
    const fetchAvailableSlots = async () => {
      if (!date || !provider?.providerId || !token) return;
      
      try {
        const res = await fetch(
          `${BACKEND_URL}/appointments/availability/${provider.providerId}/slots?date=${format(date, "yyyy-MM-dd")}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
    ...
    fetchAvailableSlots();
  }, [date, provider?.providerId, token]);
