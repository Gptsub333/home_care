...
export default function CompleteSettingsPage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      // Fetch complete provider data from the new API
      const response = await fetch(`${BACKEND_URL}/providers/${providerId}`);
      const data = await response.json();
      ...
      setFormData({
        // Basic info - now directly on Provider
        name: data.name || "",
        email: data.email || "",
        phone: data.phone || "",
  ...
  const fetchReviews = async (providerId) => {
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/providers/${providerId}/reviews?page=1&limit=5`
      );
      const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare update payload according to the new API structure
      const updatePayload = {
        name: formData.name,
      ...
      if (formData.profileImage && formData.profileImageKey) {
        ...
        updatePayload.profileImageKey = formData.profileImageKey;
      }

      // Use the new provider profile update endpoint
      const response = await fetch(`${BACKEND_URL}/providers/profile`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const updatedUser = {
        ...userData,
        ...data.provider,
        id: userData.providerId || userData.id,
      };
      localStorage.setItem("user", JSON.stringify(updatedUser));
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {review.timeAgo || getDaysAgo(review.createdAt)}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-1">
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Toggle your availability for today
                        </p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              isAvailableToday: e.target.checked,
                            })
                          }
